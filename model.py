...
# Takes files uploaded by the user and processes them based on their type (PDF, Text, CSV).
def read_data(files, loader_type):
    ...
    for file in files:
        ...
        #For PDF files, it reads each page, extracts text, and stores it as a Document object with metadata
        try:
            if loader_type == "PDF":
            ...
            #For Text files, it uses TextLoader to load the text content and metadata from the file.
            ...
            #For CSV files, it uses CSVLoader to load the content and metadata.
...
# Splits large texts into smaller chunks using RecursiveCharacterTextSplitter.
...
# Creates a vector store (FAISS) where each document chunk is stored as a vector embedding, using the OllamaEmbeddings model.
...
#Converts a document’s metadata to a dictionary for easy display
def document_to_dict(doc):
    return {
        "metadata": doc.metadata
    }

#It creates a prompt based on the system's instructions and the user’s input.
...
def main():
    st.set_page_config(page_title="LOCAL RAG", page_icon=":books:")
    st.header("Chat with PDFs using Mistral :robot_face: ")
